...
export default function PackageList() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:4000/api/shops/allshops`)
            .then((response) => {
    ...
    const onDelete = (id) => {
        axios.delete(`http://localhost:4000/api/shops/deleteshop/${id}`).then(response => {
            ...
            if (response.status === 200) {
                Swal.fire({
                    ...
                    html: '<div style="display: flex; justify-content: center; align-items: center;">' +
                        ...
                        '</div>' +
                        '<p style="text-align: center;">Shop detail has been deleted!!</p>',
                    background: '#f8f9fa',
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Unsuccessful',
                    text: "Shop has not been deleted!!",
                    background: '#f8f9fa',
    ...
    const setData = (data) => {
        let { shopId, shopName, shopOwnerName } = data;
        localStorage.setItem('Id', shopId);
        localStorage.setItem('shopName', shopName);
        localStorage.setItem('shopOwnerName', shopOwnerName);
    }
    ...
    const exportPackages = () => {
        ...
        const title = "Shop List Report";
        const headers = [["Shop ID","Shop Name", "Shop Owner Name"]];
                        ...
                        <div className='items-center overflow-hidden'>
                            <div class="grid grid-cols-1 gap-4 content-start">
                                <table className=''>
                                    ...
                                    <tr>
                                        <th className='drop-shadow-md'><h1>Shopping Management View</h1></th>
                                        <td className='flex justify-end gap-2'>
                                            <div class="flex justify-end sm:flex-row sm:text-left sm:justify-end gap-2">
                                                <button class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                                    <Link className='font-semibold text-white no-underline' to={"/add"}>
                                                        Create a new Shop
                                                    </Link></button>
                                ...
                                <table className='w-full text-lg text-left text-gray-500 dark:text-gray-400'>
                                    <thead className='p-5 text-gray-700 uppercase border bg-gray-50 dark:bg-gray-700 dark:text-gray-400'>
                                        <tr className='text-center'>
                                            <th className="p-2 border-black tbhead ">ID</th>
                                            <th className="p-2 tbhead">Shop Name</th>
                                            <th className="p-2 tbhead">Shop Owner Name</th>
                                                ...
                                                if (searchInput) {
                                                    return (
                                                        <tr className='bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'>
                                                            ...
                                                            <td className='px-6 py-4'>{data.shopOwnerName}</td>
                                                            ...
                                                            <td className='px-6 py-4'>
                                                                <div class="flex justify-center">
                                                                    <div class="">
                                                                        ...
                                                                        <button className='inline-flex items-center px-4 py-2 ml-1 text-sm font-medium text-white duration-100 bg-red-500 rounded-md hover:bg-red-200' onClick={() => onDelete(data.shopId)}>
                                                                            <div class=" grid grid-cols-2 gap-1 hover:text-black duration-100">
                                                                                <div class="">
                                                                                    <svg class="h-5 w-5 mr-2 " fill="none" viewBox="0 0 24 24" stroke="currentColor">
